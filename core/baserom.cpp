...
uint8* GetDMGRom(uint32& size)
{
    size = sizeof(GAMEBOY_CLASSIC);
	return &GAMEBOY_CLASSIC[0];
}

uint8* GetCGBRom(uint32& size)
{
    size = sizeof(GAMEBOY_COLOR);
    return &GAMEBOY_COLOR[0];
}
